...
import {visitDateLabel} from '../helpers/visitClock';

type ServiceRequestAtelierProps = {
  serviceId: string;
  onBack: () => void;
};
...
export function ServiceRequestAtelier({serviceId, onBack}: ServiceRequestAtelierProps) {
  const service = services.find(item => item.id === serviceId) ?? services[0];
  const [time, setTime] = useState('7:00 PM');
  const [notes, setNotes] = useState('');
  const [success, setSuccess] = useState(false);
  const {addReservation} = useVisitMemory();
